...
var ETH_VERSION, NET_VERSION, PROTOCOL_VERSION, API_VERSION, COINBASE;

var INSTANCE_NAME = process.env.INSTANCE_NAME || process.env.RESIN_DEVICE_NAME_AT_INIT || 'unnamed instance';
...
Node.prototype.startWeb3Connection = function () {
  ...
  web3.setProvider(
    new web3.providers.HttpProvider(
      'http://' +
      (process.env.RPC_HOST || 'localhost') +
      ':' +
...
Node.prototype.formatBlock = function (block) {
  if (
    ...
    !_.isUndefined(block.totalDifficulty)
  ) {
...
Node.prototype.validateLatestBlock = function (error, result, timeString) {
  ...
  if (error) {
    ...
    process.exit(1); // better restart, this is weird
    return false;
  }
  ...
  if (this.stats.block.number - this._lastBlock > 1) {
    ...
    if (this._latestQueue.idle()) this.getHistory({ list: range });
  }
  ...
  if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true)) {
    ...
    async.parallel(
      ...
      function (err, results) {
        ...
        console.sstats(
          '==>',
          'Got getStats results in',
          chalk.reset.cyan(results.diff, 'ms')
        );
  ...
  if (this._web3) {
    ...
    web3.eth.getBlockTransactionCount('pending', function (err, pending) {
      ...
      console.sstats(
        '==>',
        'Got',
        chalk.reset.red(pending),
        chalk.reset.bold.green(
          'pending tx' + (pending === 1 ? '' : 's') + ' in'
        ),
        chalk.reset.cyan(results.diff, 'ms')
      );
      ...
      if (self._lastPending !== pending) self.sendPendingUpdate();
...
Node.prototype.getHistory = function (range) {
  ...
  async.mapSeries(
    ...
    function (err, results) {
      ...
      self.emit('history', {
        id: self.id,
        history: results && Array.isArray(results) ? results.reverse() : []
      });
...
Node.prototype.setFilters = function () {
  ...
  this._latestQueue = async.queue(function (hash, callback) {
    var timeString =
      'Got block ' +
      chalk.reset.red(hash) +
      chalk.reset.bold.white(' in') +
      chalk.reset.green('');
  ...
  try {
    ...
    this.chainFilter.watch(function (err, hash) {
      ...
      console.stats(
        '>>>',
        'Chain Filter triggered: ',
        chalk.reset.red(hash),
        '- last trigger:',
        chalk.reset.cyan(time)
      );
      ...
      if (time < self._chan_min_time) {
        ...
        if (self._chain_debouncer_cnt > 100) {
          ...
          self._max_chain_debouncer = Math.max(
            self._max_chain_debouncer - 1,
            5
          );
        }
      } else {
      ...
      if (
        self._chainDebouncer < self._max_chain_debouncer ||
        now - self._lastBlockSentAt > 5000
      ) {
